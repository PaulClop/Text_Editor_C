...
void editorOpen(char *filename)
{
    ...
    char *line = NULL;
    size_t linecap = 0; // capacitate linie bytes
    ssize_t linelen;
    while ((linelen = getline(&line, &linecap, fp)) != -1) // citire linii multiple
    {
        while (linelen > 0 && (line[linelen - 1] == '\n' || line[linelen - 1] == '\r')) // sterege newline si carriage
            linelen--;

        editorAppendRow(line, linelen);
    }

    free(line); // eliberare memorie
    fclose(fp);
}
